...
10. 3.14 -- A new design paradigm. Before we were looking at lots of motion and full ROI changes with messy apportionments to intuit when changes were happening. This introduced two failure modes: 1) we were missing kills from kill parser, 2) we were duplicating kills in kill parser, which meant we were throwing out things we wanted and keeping garbage we didn't. Top of the funnel worked fine = all kills were accounted for. This suggests the problem was further down.
 ...
 13. 3.17 -- Reworked the killfeed pipeline to persist detector-time victim-side geometry and fingerprints instead of repeatedly re-extracting them from downstream crops. We now carry full arrow-anchored regions through the pipeline, parser reuses persisted icon geometry first, and dedupe compares victim-side identity using both the right icon and right nameplate. This improved recall substantially: all true kills from the test clip now survive into parsed_valid, but same-kill duplicates are still leaking through because the current heuristic victim fingerprints are not yet stable/discriminative enough.
     1.  Implemented identity-aware clustering so close-in-time events only merge when victim-side similarity agrees, rather than collapsing purely on time.
     2.  Persisted full arrow-anchored subregions as first-class pipeline data, including icon boxes and nameplate search regions.
     3.  Updated parser to trust persisted detector geometry before falling back to rediscovery from saved crops.
